    ...
    
{
    ...
    coarseGridNumberInX_ = ceil(mag(coarseGridVertex_.x()-coarseGridOrigin_.x())/coarseGridSize.x());
    coarseGridNumberInY_ = ceil(mag(coarseGridVertex_.y()-coarseGridOrigin_.y())/coarseGridSize.y());
    coarseGridNumberInZ_ = ceil(mag(coarseGridVertex_.z()-coarseGridOrigin_.z())/coarseGridSize.z());
    
    scalar halfCoarseGridSizeX = coarseGridSize.x()/2.0;
    scalar halfCoarseGridSizeY = coarseGridSize.y()/2.0;
    scalar halfCoarseGridSizeZ = coarseGridSize.z()/2.0;
        ...
        for (j=1;j<=coarseGridNumberInY_;j++)
            {
                ...
                {
                    coarseGridCenter.x() = coarseGridOrigin_.x() + halfCoarseGridSizeX + coarseGridSize.x()*(i-1);
                    coarseGridCenter.y() = coarseGridOrigin_.y() + halfCoarseGridSizeY + coarseGridSize.y()*(j-1);
                    coarseGridCenter.z() = coarseGridOrigin_.z() + halfCoarseGridSizeZ + coarseGridSize.z()*(k-1);
    ...
    forAll(coarseGridCellCenters_, idC)
    {
        ...
        xl_ = coarseGridCellCenters_[idC].x() - halfCoarseGridSizeX;
        xr_ = coarseGridCellCenters_[idC].x() + halfCoarseGridSizeX;
        yl_ = coarseGridCellCenters_[idC].y() - halfCoarseGridSizeY;
        yr_ = coarseGridCellCenters_[idC].y() + halfCoarseGridSizeY;
        zl_ = coarseGridCellCenters_[idC].z() - halfCoarseGridSizeZ;
        zr_ = coarseGridCellCenters_[idC].z() + halfCoarseGridSizeZ;
...
) const
{
    ...
    {       
        ...
        if (fineCellNumber > 0)
        {
            forAll(finIdList, idF)
            {
                sumx = sumx + fine[finIdList[idF]].x()*weight[finIdList[idF]];
                sumy = sumy + fine[finIdList[idF]].y()*weight[finIdList[idF]];
                sumz = sumz + fine[finIdList[idF]].z()*weight[finIdList[idF]];
                sumWeight = sumWeight + weight[finIdList[idF]];
            }
...
//average scalar fields weighted by cell volume
tmp<volScalarField>
Foam::coarserGrid::coarserGrid::averagedSourceField
(
...
Foam::tmp<volScalarField::Internal>
Foam::coarserGrid::coarserGrid::averagedSource
(
    const volScalarField::Internal& fineField, 
    volScalarField weightField
) const
{
    ...
    (
        ...
        (
            "averagedFieldTmp",
            mesh_,
            dimensionedScalar(fineField.dimensions(), 0.0)
        )
    );
...
Foam::coarserGrid::coarserGrid::averagedSource
(
    const volVectorField::Internal& fineField, 
    volScalarField weightField
) const
{
    ...
    (
        ...
        (
            "averagedFieldTmp",
            mesh_,
            dimensionedVector(fineField.dimensions(), Zero)
        )
    );
